...
const COPY: Record<Locale, CopyShape> = {
  en: {
    ...
    pasteBackLabel: (display) => `Paste this RentHome Departamentos URL back into ${display}`,
  ...
  de: {
    ...
    pasteBackLabel: (display) => `Diese RentHome Departamentos-URL zurück in ${display} einfügen`,
  ...
  fr: {
    ...
    pasteBackLabel: (display) => `Recollez cette URL RentHome Departamentos dans ${display}`,
  ...
  es: {
    ...
    pasteBackLabel: (display) => `Pegue esta URL de RentHome Departamentos de vuelta en ${display}`,
...
function PlatformRow({
  ...
  return (
    <div className="rounded-lg border border-[var(--line)] bg-[var(--bg)] transition-colors hover:border-[var(--line-2)]">
      ...
      {/* Body — URL inputs + test + RentHome Departamentos URL — only when enabled */}
      {row.enabled && (
        <div className="border-t border-[var(--line)] px-4 py-4 space-y-3">
          ...
          {/* RentHome Departamentos feed URL for this platform */}
          <div>
            <label className="block text-[12px] font-medium text-[var(--ink-2)] mb-1.5">
