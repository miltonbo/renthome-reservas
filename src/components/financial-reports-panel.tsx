...
function ManagementReport({ data, channels }: { data: ReportData; channels: Array<any> }) {
  return <div className="mt-4 space-y-3"><div className="overflow-x-auto rounded-xl border border-[var(--line)] bg-[var(--bg)]"><table className="w-full text-xs"><thead className="bg-[var(--bg-3)] text-[var(--ink-4)]"><tr><th className="px-3 py-2 text-left">Canal</th><th className="px-3 py-2 text-right">Reservas</th><th className="px-3 py-2 text-right">Total Bs</th><th className="px-3 py-2 text-right">Total USD</th></tr></thead><tbody>{channels.map(row => <tr key={row.channel} className="border-t border-[var(--line)]"><td className="px-3 py-2 font-semibold"><span className="mr-2 inline-block h-2 w-2 rounded-full" style={{backgroundColor:row.color}}/>{row.label}</td><td className="px-3 py-2 text-right">{row.reservations}</td><td className="px-3 py-2 text-right">{amount(row.BOB,"BOB")}</td><td className="px-3 py-2 text-right">{amount(row.USD,"USD")}</td></tr>)}</tbody></table></div><div className="grid gap-3 sm:grid-cols-2">{(["deysi","milton"] as const).map(person => <div key={person} className="rounded-xl border border-[var(--line)] bg-[var(--bg)] p-3"><div className="font-semibold capitalize">{person}</div><div className="mt-2 grid grid-cols-2 gap-3 text-xs"><div><span className="text-[var(--ink-4)]">Recibido</span><strong className="mt-1 block">{money(data.held[person].BOB,"BOB")}</strong><strong className="block">{money(data.held[person].USD,"USD")}</strong></div><div><span className="text-[var(--ink-4)]">Le corresponde</span><strong className="mt-1 block">{money(data.entitled[person].BOB,"BOB")}</strong><strong className="block">{money(data.entitled[person].USD,"USD")}</strong></div></div><div className="mt-3 border-t border-[var(--line)] pt-2 text-xs text-[var(--ink-3)]">Comisión Booking pendiente: <strong>{money(data.commissions[person].BOB,"BOB")}</strong> · <strong>{money(data.commissions[person].USD,"USD")}</strong></div></div>)}</div><div className="rounded-xl bg-[var(--brand-orange-soft)] p-3 text-xs"><div className="font-semibold">Conciliación resultante</div>{data.transfers.length ? data.transfers.map(item => <div key={`${item.from}-${item.currency}`} className="mt-1 capitalize">{item.from} transfiere a {item.to}: <strong>{money(item.amountMinor,item.currency)}</strong></div>) : <p className="mt-1 text-[var(--ink-3)]">No existen transferencias pendientes para este período.</p>}</div></div>;
}
