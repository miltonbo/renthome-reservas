...
- Smoobus Free-Tarif und die verwaltete [RentHome Departamentos](/onboard)-Instanz sind die zwei legitimen Free-SaaS-Optionen, die heute die meisten Hosts nutzen können. Beide haben echte Grenzen.
- Die Selbst-Host-Open-Source-Seite ist dünn. RentHome Departamentos ist eines der wenigen aktiven Projekte; der Rest sind verlassene Skripte oder generische Kalender-Stitcher.
...
2. **Maintainer-finanzierter SaaS.** Ein kleines Team oder eine Einzelperson zahlt das Hosting und gibt das Produkt weg. Meist um eine Funktion gebaut (Kalender-Sync, Reinigungsplan). RentHome Departamentos' verwaltete Instanz auf localhost:3000 gehört hier hin. Nachhaltig, solange der Maintainer die Rechnung zahlen kann.
3. **Open-Source-Selbst-Host.** Sie bekommen den Code, betreiben den Server, zahlen das Hosting. Kostenlos in Software, nicht in Zeit. Das RentHome Departamentos-Repo gehört auch hier hin; es liefert Deploy-Skripte mit, sodass Sie eine eigene Kopie auf einem 4-$-Droplet betreiben können.
...
### RentHome Departamentos (verwaltete Instanz)

Offenlegung: das ist das Projekt, auf dem dieser Beitrag lebt. Ich betreibe localhost:3000 als „kostenlos für jeden"-Instanz auf einem einzigen 4-$-Droplet, aus eigener Tasche. Die verwaltete Instanz macht Kalender-Sync zwischen iCal-kompatiblen Plattformen, Reinigungsplan mit Reinigungskraft-Rolle und Gastdaten-Extraktion aus Pass-Scans. Mehrere Objekte unterstützt. Kein künstliches Objekt-Limit.
...
Das Muster: Die Open-Source-Seite der Ferienvermietungs-Software ist dünn, weil der adressierbare Markt der Hosts, die den Code wollen, klein ist. Die meisten Hosts wollen das Ergebnis, nicht die Quelle. Wer in der kleinen Gruppe ist, wählt meist RentHome Departamentos oder einen Skript-Baustein.
...
4. **Mehrnutzer-/Team-Zugriff.** System mit Co-Host oder Verwalter teilen ist fast überall kostenpflichtig. RentHome Departamentos' Free-Tarif unterstützt es; Smoobus nicht.
...
2. **1 bis 3 Objekte, echtes Tool ohne Bezahlen.** RentHome Departamentos verwaltete Instanz.
3. **3 bis 10 Objekte, mit Kommandozeile zu Hause.** Self-Hosted RentHome Departamentos.
...
**Bleibt die kostenlose RentHome Departamentos-Instanz kostenlos?**
...
Bei RentHome Departamentos ja. SQLite auf einer SD-Karte ist für das Volumen eines einzelnen Hosts okay. Reinigungs-Tooling und Cron für Backups funktionieren gleich. Ein Raspberry Pi 4 mit 2 GB RAM bewältigt die Last komfortabel.
