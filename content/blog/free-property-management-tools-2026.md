...
- Smoobu's free plan and the managed instance of [RentHome Departamentos](/onboard) are the two legitimate free SaaS options most hosts can use today. Both have real limits.
- The self-hostable open-source side is thin. RentHome Departamentos is one of the only active projects; the rest are abandoned scripts or generic calendar stitchers.
...
2. **Maintainer-funded SaaS.** A small team or individual pays for the hosting and gives the product away. Usually built around a single feature (calendar sync, cleaning schedule). RentHome Departamentos' managed instance at localhost:3000 belongs here. Sustainable as long as the maintainer can pay the bill.
3. **Open-source self-host.** You get the code, you run the server, you pay the hosting. Free in software, not in time. The RentHome Departamentos repo is in this group too; it ships with deployment scripts so you can run your own copy on a $4 droplet.
...
The pattern: the open-source side of vacation-rental software is sparse because the addressable market of hosts who want the code is small. Most hosts want the result, not the source. If you are in the small group, the choice is usually RentHome Departamentos or one of the script-level building blocks.
...
4. **Multi-user / team access.** Sharing the system with a co-host or property manager is paid almost everywhere. RentHome Departamentos' free tier supports it; Smoobu's does not.
...
2. **One to three properties, want a real tool but not paying.** RentHome Departamentos managed instance.
3. **Three to ten properties, fine with the command line.** Self-hosted RentHome Departamentos.
...
**Will the free RentHome Departamentos instance stay free?**
...
Yes for RentHome Departamentos. SQLite on an SD card is fine for a single host's volume. The cleaning-tooling and the cron job for backups all work the same. A Raspberry Pi 4 with 2 GB of RAM handles the workload comfortably.
...
The free-tools landscape is a snapshot in time and the snapshot will look different in 18 months. The tools most likely to still be around in 2027 are the ones with a sustainable cost structure — which usually means a paid tier propping up the free tier (Smoobu) or a small, focused scope that one maintainer can run on $5 a month (RentHome Departamentos).
