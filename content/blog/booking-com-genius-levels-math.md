...
locale: en
title: "Booking.com Genius levels: when 10%, 15%, and 20% discounts pay back"
excerpt: Worked math on Booking.com's Genius 1, 2, and 3 tiers. The breakeven lift each tier needs, the visibility math behind the badge, and a per-listing rule.
status: published
tags:
  - host-tips:Host tips
  - pricing:Pricing
...
- Genius is a **loyalty programme for guests, paid for by hosts** — Booking awards the badge to repeat bookers and you cover the discount.
- Three tiers: Level 1 = 10%, Level 2 = 15%, Level 3 = 20%. Each tier requires a higher minimum: 3, 10, and 30 bookings in 24 months.
- Breakeven for the 10% Genius tier is roughly **+12% extra bookings** that wouldn't exist without the badge. Most listings get +4–8%.
- Genius **stacks multiplicatively** with your length-of-stay rate plans and seasonal discounts. A 20% monthly + 10% Genius is **28% off**, not 30%.
- Above 80% occupancy Genius is dead weight; below 60% on a non-Genius listing it usually pays. The middle band is where you measure.
- The Genius opt-in is **always renewable**. Try it for 60 days, measure, decide. Don't decide on day zero.
...
For a deeper dive on length-of-stay discount math itself, see [length-of-stay discount math](/blog/length-of-stay-discount-math).
...
1. **Your occupancy crossed 85%.** You don't need badge visibility anymore. The discount is pure pay cut.
2. **Your average daily rate rose more than 20% in the last quarter.** Higher ADR means each Genius booking costs more in absolute dollars. The lift% the badge provides is roughly constant; the cost grows linearly with rate.
3. **You're running a length-of-stay rate plan or seasonal promotion.** Stacking compounds. Either drop Genius or drop the other promo — running both simultaneously is the easiest way to wake up at 30% effective discount on a slow Tuesday in February.
...
If you're already running on both Airbnb and Booking, the visibility lever on either platform matters less because guests cross-shop. See [the free iCal sync setup](/blog/airbnb-booking-calendar-sync-free) for the prerequisite to running both without double-bookings.
