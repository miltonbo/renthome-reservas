...
locale: en
title: "Who collects tourist tax on Airbnb, Booking.com, and Vrbo (2026)"
excerpt: "A practical map of where Airbnb, Booking.com, and Vrbo auto-collect tourist tax — and where they leave you on the hook. Rates by US city and EU country."
status: published
tags:
  - airbnb:Airbnb
...
- **Airbnb auto-collects in ~30 US states, ~150 cities, and a growing list of EU countries**; the rest is on the host.
- **Booking.com auto-collects in fewer than 25 jurisdictions worldwide**; the default is "guest pays at property," which means *you* file and remit.
- **Vrbo's coverage roughly tracks Airbnb's in the US** but is patchy in Europe; check the "Lodging tax" line on the host dashboard per listing.
- The same property can have tax **auto-collected on Airbnb and not on Booking.com** — the agreements are jurisdiction-by-jurisdiction and platform-by-platform.
- "Auto-collected" doesn't always mean "auto-remitted." In ~12 US states, the platform collects but hands the money to the host with a Form 1099 line; the host still has to remit to the city.
- Penalties for non-remittance run **5–25% of the tax owed plus interest**; chronic non-filing in EU markets adds **€500–€5,000 per quarter** of fixed penalties on top.
- Every host with 3+ listings or multi-platform coverage should run a **quarterly remittance reconciliation**: pull each platform's tax report, cross-check against the local authority's portal, file the gap.
...
**United States** — approximately 30 states either through a state-level marketplace facilitator law or city-by-city agreements. The full list is in [Airbnb's "Occupancy tax collection" help page](https://www.airbnb.com/help/article/653) under the "In which jurisdictions does Airbnb collect and remit occupancy taxes" section. The high-volume ones to know:

| Jurisdiction | Tax rate | Auto-remitted |
| --- | --- | --- |
| California (state) | 0% state, varies by city (10–15% combined typical) | Yes, in ~100 cities |
| New York City | 14.75% combined (4% sales + 5.875% hotel + others) | Yes |
| Florida (state) | 6% state sales + 0–6% county tourist development | Yes in 49 of 67 counties |
| Texas (state) | 6% state hotel occupancy + local | Yes in ~80 cities |
| Hawaii | 10.25% TAT + 3% county surcharge | Yes |
| Washington DC | 14.95% sales | Yes |

**European Union and EFTA** — auto-collection covers approximately 35 countries and regions:

| Country / region | Typical rate | Auto-remitted |
| --- | --- | --- |
| Portugal (Lisbon, Porto, Madeira) | €2/night per guest, capped at 7 nights | Yes |
| Spain (Catalonia, Valencia, Balearics) | €0.45–€3.50/night per guest by region | Yes in Catalonia + Valencia; partial in Balearics |
| France (most cities) | 1–5% of nightly rate; capped € floor by star rating equivalent | Yes |
| Italy (Rome, Florence, Milan, Venice) | €1–€7/night per guest by city + property type | Yes |
| Germany (Berlin, Hamburg, Munich) | 5% of net stay cost in Berlin; €3–€5/night elsewhere | Yes in ~40 cities |
| Netherlands (Amsterdam) | 12.5% of nightly rate | Yes |
| Greece (national) | €0.50–€10/night by property class + season | Yes |
...
- **Approximately 12 US cities and 4 US states** — full list on Booking's [partner help](https://partner.booking.com/en-us/help/legal-security/taxes-payments) page, search "marketplace facilitator."
...
For the broader operational picture once you start tracking tax line items alongside everything else, see [GDPR for vacation rental hosts](/blog/gdpr-for-vacation-rental-hosts) — the same quarterly cadence works for data-protection compliance.
...
2. **Set the rate on the listing.** On Booking.com, this is **Property → Property setup → Taxes and charges**. Add a city tax line at the local rate. The guest sees it at checkout, the money flows to your payout. On Vrbo, this is **Property setup → Rates and fees → Lodging tax**.

3. **File on the local cadence.** EU cities are usually monthly or quarterly; US states are usually monthly, quarterly, or annually depending on volume. The portal accepts a CSV upload of the reservation list; total tax owed is the per-night rate × occupied nights × guest count for per-guest taxes, or the percentage × room revenue for percentage taxes. The portal calculates, you pay.
